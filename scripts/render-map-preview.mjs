...
const maps = {
  "eld-library": {
    ...
    fixtures: [
      ...
      ["counter",2,13,1,3],["stairs",0,15,1,1],["desk",9,12,1,4],
    ],
    stools: [[8,12],[8,13],[8,14],[8,15]],
    characters: [{kind:"librarian",tile:[1,14],facing:"right"}],
    markers: [
      {kind:"book",tile:[3,4],label:"BOOK"},
      {kind:"bookmark",tile:[9,14],label:"No.01"},
    ],
    entrance: {xTiles:[4,5]},
  },
};
...
const markers = map.markers.map(m => { const [x,y]=center(m.tile); return `<g><rect class="marker ${m.kind}" x="${x-24}" y="${y-11}" width="48" height="22"/><text x="${x}" y="${y+4}" text-anchor="middle">${esc(m.label)}</text></g>`; }).join("\n");
...
${chars}
${markers}
<rect class="entrance" x="${entranceX}" y="${map.height-20}" width="${entranceW}" height="20"/>
<line x1="${entranceX}" y1="${map.height-20}" x2="${entranceX+entranceW}" y2="${map.height-20}" stroke="#8d7650" stroke-width="2"/>
</svg>`;

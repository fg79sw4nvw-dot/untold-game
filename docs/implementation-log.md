...
- No.01「忘れられた栞」とNo.55「書喰い鼠」の確定カード番号、およびNo.55の3情報に対応する内部 `clueId` をデータ定義として追加。

### 今回あえて実装していない内容

- No.55の3情報を話す具体的なNPCはWikiで未確定のため配置・NPCデータ化していない。
- No.55の「特定ポイント」の具体座標は未確定のため取得地点判定へ接続していない。
- 蜜漬け木の実の正式なアイテム定義・分類・所持データは未確定部分が残るため、実カードデータへ接続していない。
- 依頼システムの状態遷移ルールは独立した確定仕様が不足しているため、イベント状態を参照できる受け口だけ用意し、依頼状態を勝手に定義していない。
...
この実行環境からGitHubへのネットワーク接続ができず、ローカルcloneによる `npm test` / `npm run build` の再実行はできなかった。今回追加したファイルは既存のTypeScript型定義に合わせて実装しているが、次回GitHub Actionsまたは実行可能環境で型チェックとテストを再確認する。
